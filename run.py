...
model=get_model()
# torch.save(model, './bert_base_chinese/test'+'.pth')
# exit()
# exit()
...
def train(net,dataset,num_epochs, learning_rate,  batch_size):
    net.train()
    optimizer = optim.SGD(net.parameters(), lr=learning_rate,weight_decay=0)#构建优化器
...
if __name__ == '__main__':
    ...
    train(model, train_dataset, 10, 0.002, 8)  # 开始训练参数如下：预训练模型、训练集、epoch、学习率、batch—size
    # #eval(model,dev_dataset,8)